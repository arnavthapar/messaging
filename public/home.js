    ...
    try {
        ...
        for (const message of data.messages) {
            if (prevMessage && prevMessage.sender === message.sender && prevMessage.datetime === message.datetime) {
                ...
                pval.style.margin = "0px";
                pval.textContent = message.content;
                h3.textContent = message.sender + " - " + new Date(message.date).toLocaleString(message.datetime, {month: 'numeric', day: 'numeric',  year: 'numeric', hour: 'numeric', minute: '2-digit', hour12: true }),
